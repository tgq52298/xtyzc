function checkusername() {
	...
	if (!namez.test(myform.name.value)) {
		...
		errMsg_nickname.innerHTML = "<font color='red'>姓名长度为2-5</font>";
		return false;
	} else {
...
function checkemail() {
	...
	if (!mail.test(myform.email.value)) {
		errMsg_email.innerHTML = "<font color='red'>请输入正确邮箱</font>";
		return false;
	} 
	else {
		errMsg_email.innerHTML = "<font color='green'>邮箱正确</font>";
		return true;
	}
}
...
function checkform() {
	...
	checkemail();
	checkcontent();
	if (checkusername() && checkphone() && checkemail()&&checkcontent()) {
		return true;
	} else {
		return false;
	}
...
var aa =document.getElementById("form1").onsubmit=function(){
	 return checkform()
}
